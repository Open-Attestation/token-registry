...
describe("TradeTrustERC721 (TS Migration)", async () => {
  ...
  /* describe("Impermanent burning of tokens", () => {
    ...
    it("should revert if caller is not a minter", async () => {
      const tx = tradeTrustERC721Mock.connect(users.holder).burn(tokenId);

      await expect(tx).to.be.revertedWith("MinterRole: caller does not have the Minter role");
    });

    it("should revert if caller is an unapproved minter", async () => {
      const tx = tradeTrustERC721Mock.connect(users.carrier).burn(tokenId);

      await expect(tx).to.be.revertedWith("TokenRegistry: Caller is not owner nor approved");
    });

    describe("When caller is an approved minter", () => {
      let tradeTrustERC721MockAsApprovedMinter: TradeTrustERC721Mock;

      beforeEach(async () => {
        await tradeTrustERC721Mock.connect(users.beneficiary).approve(users.carrier.address, tokenId);
        tradeTrustERC721MockAsApprovedMinter = tradeTrustERC721Mock.connect(users.carrier);
      });

      describe("When a token has been surrendered", () => {
        it("should revert when minter burns a token", async () => {
          await tradeTrustERC721Mock
            .connect(users.beneficiary)
            ["safeTransferFrom(address,address,uint256)"](
              users.beneficiary.address,
          ...
          const tx = tradeTrustERC721MockAsApprovedMinter.burn(tokenId);
      ...
      describe("When a token has not been surrendered", () => {
        it("should burn token to zero address", async () => {
          await tradeTrustERC721MockAsApprovedMinter.burn(tokenId);

          const tx = tradeTrustERC721MockAsApprovedMinter.ownerOf(tokenId);
    ...
    describe("When a token has been impermanently burnt", () => {
      let tradeTrustERC721MockAsApprovedMinter: TradeTrustERC721Mock;
      ...
      it("should allow mintTitle on the same token ID again", async () => {
        await tradeTrustERC721MockAsApprovedMinter.mintTitle(users.beneficiary.address, users.holder.address, tokenId);
