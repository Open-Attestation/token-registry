...
describe("TradeTrustErc721", async () => {
  ...
  it("should work without a wallet for read operations", async () => {
    ...
    await tokenRegistryInstanceWithShippingLine["mint(address,uint256)"](owner1.address, merkleRoot);
  ...
  it("should be able to mint", async () => {
    ...
    await tokenRegistryInstance["mint(address,uint256)"](owner1.address, merkleRoot);
  ...
  it("should be able to transfer", async () => {
    ...
    await tokenRegistryInstanceWithShippingLineWallet["mint(address,uint256)"](owner1.address, merkleRoot);
  ...
  describe("Surrendered TradeTrustERC721 Work Flow", () => {
    ...
    it("token cannot be destroyed if not owned by registry", async () => {
      ...
      await expect(attemptDestroyToken).to.be.rejectedWith(
        "VM Exception while processing transaction: reverted with reason string 'Cannot destroy token: Token not owned by token registry'"
      );
    });

    it("should be able to send token owned by registry", async () => {
      await tokenRegistryInstanceWithShippingLineWallet["sendToken(address,uint256)"](owner1.address, merkleRoot);
      const currentOwner = await tokenRegistryInstanceWithShippingLineWallet.ownerOf(merkleRoot);
      expect(currentOwner).to.deep.equal(owner1.address);
    });
    ...
    it("minter should not be able to send token not owned by registry", async () => {
      await tokenRegistryInstanceWithShippingLineWallet["safeMint(address,uint256)"](owner1.address, merkleRoot1);
      const attemptSendToken = tokenRegistryInstanceWithShippingLineWallet["sendToken(address,uint256)"](
        owner2.address,
        merkleRoot1
      );
      await expect(attemptSendToken).to.be.rejectedWith(
        "VM Exception while processing transaction: reverted with reason string 'Cannot send token: Token not owned by token registry'"
      );
    });
    ...
    it("should be able to send token to new title escrow", async () => {
      ...
      await tokenRegistryInstanceWithShippingLineWallet.sendToNewTitleEscrow(
        owner1.address,
        holder1.address,
        merkleRoot
      );
