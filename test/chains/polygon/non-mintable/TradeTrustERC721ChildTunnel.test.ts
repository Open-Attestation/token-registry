...
describe("TradeTrustERC721ChildTunnel - Non-mintable Tokens", () => {
  let stubFxChildMock: MockContract<FxChildMock>;

  let stubRootToken: MockContract<TradeTrustERC721>;
  let stubChildToken: MockContract<TradeTrustERC721Child>;
  ...
  describe("ERC721 Receiver", () => {
    it("should be able to receive token", async () => {
      ...
      const titleEscrowSigner = await impersonateAccount({ address: titleEscrowAddr });
      await stubChildToken
        .connect(titleEscrowSigner)
        ["safeTransferFrom(address,address,uint256)"](
          titleEscrowAddr,
          tradeTrustERC721ChildTunnelMock.address,
          tokenId
        );
