...
export const deployMintableFixture =
  ...
  async () => {
    ...
    // Deploy root token
    const stubTradeTrustERC721Factory = await smock.mock<TradeTrustERC721RootMintable__factory>(
      "TradeTrustERC721RootMintable"
    );
    const stubRootToken: MockContract<TradeTrustERC721RootMintable> = await stubTradeTrustERC721Factory.deploy(
    ...
    // Deploy child token
    const stubTradeTrustERC721ChildFactory = await smock.mock<TradeTrustERC721ChildMintable__factory>(
      "TradeTrustERC721ChildMintable"
    );
    const stubChildToken: MockContract<TradeTrustERC721ChildMintable> = await stubTradeTrustERC721ChildFactory.deploy(
      "The Great Shipping Company",
      "GSC"
    );
